...
async def test_inapp_notification():
    result = await notification_service.send_notification("test_user", "Test message", "inapp")
...
async def test_email_notification(mock_post):
    ...
    result = await notification_service.send_notification("test_user", "Test email", "email")
    assert result["status"] == "notification sent"
...
async def test_invalid_notification_method():
    with pytest.raises(ValueError, match="Invalid notification method"):
        await notification_service.send_notification("test_user", "Test message", "invalid")
