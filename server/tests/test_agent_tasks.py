...
def mock_task_manager():
    ...
    manager.wallet.update_balance = AsyncMock(return_value=WalletModel(user_id="test_user", 
                                                                      balance=10.0))
...
async def test_execute_train_vial(client, mock_task_manager):
    with patch("server.services.agent_tasks.AgentTaskManager", return_value=mock_task_manager):
        response = client.post("/agent/task", json={"task_id": "train_vial", 
...
async def test_execute_git_commit(client, mock_task_manager):
    with patch("server.services.agent_tasks.AgentTaskManager", return_value=mock_task_manager):
        response = client.post("/agent/task", json={"task_id": "git_commit", 
